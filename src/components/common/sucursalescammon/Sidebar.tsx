...
export const MENU_ITEMS = [
    ...
    { id: 2, label: "Inventario", route: "/inventario/", icon: ClipboardDocumentListIcon },
    { id: 3, label: "Reportes", route: "/reportes/", icon: ChartPieIcon },
    { id: 5, label: "Citas", route: "/citas/", icon: CalendarDaysIcon },
    { id: 6, label: "Empleados", route: "/empleados/", icon: UserGroupIcon },
];
                    ...
                    <ul className='flex flex-col gap-2'>
                        {
                            MENU_ITEMS.map((item) => (
                                <li key={item.id} className='hover:bg-blue-950 rounded-md duration-150 h-10'>
                                    <Link href={`${item.route}${id}`} className='h-full capitalize flex items-center gap-2 text-blue-950 *:hover:text-white'>
